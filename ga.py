    ...
    try:
        with open(path, 'r', encoding='utf-8', errors='replace') as f:
            ...
            if show_linenos:
                ...
                result += (
                    f"\n\n[FILE PARTIAL: showing {realcnt}/{tl_str} lines; "
...
class GenericAgentHandler(BaseHandler):
    ...
    def __init__(self, parent, last_history=None, cwd='./temp'):
        ...
        self.code_stop_signal = []  # 停止代码执行的信号
        self._done_hooks = []
        ...
        if len(blocks) == 1:
            ...
            if not after_block.strip():
                ...
                if len(clean_residual) <= 30:
                    yield (
                        "[Info] Detected large code block without tool call and no extra "
...
''' + get_global_memory()
    ...
    def turn_end_callback(self, response, tool_calls, tool_results, turn, next_prompt, exit_reason):
        ...
        if rsumm:
            ...
            args = tc['args']   # at least one because no_tool
        ...
        if _plan and turn >= 10 and turn % 5 == 0:
            next_prompt = (
                f"[Plan Hint] 正在计划模式。必须 file_read({_plan}) 确认当前步骤，"
                "回复开头引用：📌 当前步骤：...\n\n"
                + next_prompt
            )
        if _plan and turn >= 90:
...
def get_global_memory():
    ...
    try:
        ...
        with open(
            ...
            insight = f.read()
        with open(
            os.path.join(script_dir, f'assets/insight_fixed_structure{suffix}.txt'),
            'r',
            encoding='utf-8',
        ) as f:
            structure = f.read()
