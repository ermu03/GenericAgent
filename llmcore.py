...
def reload_mykeys():
    ...
    if mk.get("langfuse_config"):
        try:
            from plugins import langfuse_tracing
        except Exception:
            pass
    return mk, True
...
def compress_history_tags(messages, keep_recent=10, max_len=800, force=False):
    ...
    def _trunc(text):
        ...
        for pat in _pats.values():
            text = pat.sub(
                lambda m: m.group(1) + _trunc_str(m.group(2)) + m.group(3), text
            )
        return text
    ...
    print(
        f"[Cut] {_before} -> {sum(len(json.dumps(m, ensure_ascii=False)) for m in messages)}"
    )
    return messages
...
def trim_messages_history(history, context_win):
    ...
    if cost > context_win * 3:
        compress_history_tags(
            history, keep_recent=4, force=True
        )  # trim breaks cache, so compress more btw
        ...
        while len(history) > 5 and cost > target:
            ...
            cost = sum(len(json.dumps(m, ensure_ascii=False)) for m in history)
        print(
            f"[Debug] Trimmed context, current: {cost} chars, {len(history)} messages."
        )
    ...
    for line in resp_lines:
        ...
        if evt_type == "message_start":
            ...
            if delta.get("type") == "text_delta":
                ...
                if current_block and current_block.get("type") == "thinking":
                    current_block["signature"] = current_block.get(
                        "signature", ""
                    ) + delta.get("signature", "")
    ...
    if api_mode == "responses":
        ...
        for idx in sorted(fc_buf):
            ...
            for i, inp in enumerate(inps):
                ...
                blocks.append(
                    {"type": "tool_use", "id": bid, "name": fc["name"], "input": inp}
                )
        return blocks
    else:
        ...
        for idx in sorted(tc_buf):
            ...
            for i, inp in enumerate(inps):
                ...
                if len(inps) > 1:
                    bid = f"{bid}_{i}" if bid else f"split_{i}"
                blocks.append(
                    {"type": "tool_use", "id": bid, "name": tc["name"], "input": inp}
                )
        return blocks
    ...
    for attempt in range(sess.max_retries + 1):
        ...
        try:
            with requests.post(
                ...
                json=payload,
                stream=sess.stream,
                timeout=(sess.connect_timeout, sess.read_timeout),
                ...
                if r.status_code >= 400:
                    if r.status_code in _RETRYABLE and attempt < sess.max_retries:
                        d = _delay(r, attempt)
                        print(
                            f"[LLM Retry] HTTP {r.status_code}, retry in {d:.1f}s ({attempt + 1}/{sess.max_retries + 1})"
                        )
                        time.sleep(d)
                        continue
                    try:
                        ...
                        body = ""
                    err = f"!!!Error: HTTP {r.status_code}" + (
                        f": {body}" if body else ""
                    )
                    yield err
            ...
            if attempt < sess.max_retries:
                ...
                print(
                    f"[LLM Retry] {type(e).__name__}, retry in {d:.1f}s ({attempt + 1}/{sess.max_retries + 1})"
                )
                yield err
                time.sleep(d)
                continue
...
def _openai_stream(sess, messages):
    ...
    if "kimi" in ml or "moonshot" in ml:
        ...
        temperature = max(
            0.01, min(temperature, 1.0)
        )  # MiniMax requires temp in (0, 1]
    headers = {
    ...
    if api_mode == "responses":
        ...
        payload = {
            ...
            "stream": sess.stream,
            "prompt_cache_key": _RESP_CACHE_KEY,
        ...
        if sess.max_tokens:
            payload[
                ...
                if ml.startswith(("gpt-5", "o1", "o2", "o3", "o4"))
                else "max_tokens"
            ] = sess.max_tokens
        if sess.reasoning_effort:
    ...
    parse_fn = (
        (lambda r: _parse_openai_sse(r.iter_lines(), api_mode))
        if sess.stream
        else (lambda r: _parse_openai_json(r.json(), api_mode))
    )
    ...
    def __init__(self, cfg):
        ...
        def _enum(key, valid):
            ...
            return (
                ...
                else print(f"[WARN] Invalid {key} {v!r}, ignored.")
            )

        self.reasoning_effort = _enum(
            "reasoning_effort", {"none", "minimal", "low", "medium", "high", "xhigh"}
        )
        self.service_tier = _enum(
        ...
        mode = (
            str(cfg.get("api_mode", "chat_completions"))
            .strip()
            .lower()
            .replace("-", "_")
        )
        self.api_mode = (
            "responses" if mode in ("responses", "response") else "chat_completions"
        )
    ...
    def _apply_claude_thinking(self, payload):
        ...
        if self.reasoning_effort:
            ...
            if effort:
                ...
                print(
                    f"[WARN] reasoning_effort {self.reasoning_effort!r} is unsupported for Claude output_config.effort, ignored."
                )
...
def _ensure_thinking_blocks(messages, model):
    ...
    for m in messages:
        ...
        if not isinstance(c, list):
            continue
        has_thinking = any(
            isinstance(b, dict) and b.get("type") == "thinking" for b in c
        )
        if not has_thinking:
            m["content"] = [
                {"type": "thinking", "thinking": "...", "signature": "placeholder"},
                *c,
            ]
    return messages
...
class ClaudeSession(BaseSession):
    def raw_ask(self, messages):
        ...
        parse_fn = (
            (lambda r: _parse_claude_sse(r.iter_lines()))
            if self.stream
            else (lambda r: _parse_claude_json(r.json()))
        )
    ...
    def make_messages(self, raw_list):
        ...
        for idx in user_idxs[-2:]:
            msgs[idx]["content"][-1] = dict(
                msgs[idx]["content"][-1], cache_control={"type": "ephemeral"}
            )
        return msgs
...
def _fix_messages(messages):
    ...
    for m in messages:
        ...
        if fixed and fixed[-1]["role"] == "assistant" and m["role"] == "user":
            uses = [
                b.get("id")
                for b in fixed[-1].get("content", [])
                if isinstance(b, dict) and b.get("type") == "tool_use" and b.get("id")
            ]
            has = {
                b.get("tool_use_id")
                for b in _wrap(m["content"])
            ...
            if orphan:
                m = {
                    **m,
                    "content": [
                        {"type": "text", "text": str(b.get("content", ""))}
                        if isinstance(b, dict)
                        and b.get("type") == "tool_result"
                        and b.get("tool_use_id") in orphan
                        else b
                        for b in _wrap(m["content"])
                    ],
                }
...
class NativeClaudeSession(BaseSession):
    ...
    def raw_ask(self, messages):
        messages = _ensure_thinking_blocks(
            _drop_unsigned_thinking(_fix_messages(messages)), self.model
        )
        if self.max_tokens is None:
            self.max_tokens = 8192
        model = self.model
    ...
    def ask(self, msg):
        ...
        with self.lock:
            # 把 merged 消息加入 history
            self.history.append(msg)
            # 裁剪历史
            trim_messages_history(self.history, self.context_win)
            messages = [
                {"role": m["role"], "content": list(m["content"])} for m in self.history
            ]
        # 收集 content_blocks
        content_blocks = None
        gen = self.raw_ask(messages)
        try:
            ...
            content_blocks = e.value or []
        if content_blocks and (_injected := _ensure_text_block(content_blocks)):
            yield _injected
        if content_blocks and not (
            len(content_blocks) == 1
            and content_blocks[0].get("text", "").startswith("!!!Error:")
        ):
        ...
        thinking_parts = [
            b["thinking"] for b in content_blocks if b.get("type") == "thinking"
        ]
...
"""
    ...
    def _parse_mixed_response(self, text):
        ...
        think_match = re.search(
            r"<think(?:ing)?>(.*?)</think(?:ing)?>", text, re.DOTALL
        )
        if think_match:
            thinking = think_match.group(1).strip()
        ...
        if not tool_calls:
            ...
            for json_str in json_strs:
                try:
                    ...
                    func_name = (
                        data.get("name") or data.get("function") or data.get("tool")
                    )
                    args = (
                        data.get("arguments")
                        or data.get("args")
...
def _write_llm_log(label, content):
    log_dir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "temp/model_responses"
    )
...
class MixinSession:
    ...
    def __init__(self, all_sessions, cfg):
        self._retries, self._base_delay = (
            cfg.get("max_retries", 3),
            cfg.get("base_delay", 1.5),
        )
        ...
        self._sessions = [
            ...
            if isinstance(i, int)
                ...
                if type(s) is not dict and s.backend.name == i
            )
    ...
    _BROADCAST_ATTRS = frozenset(
        {"system", "tools", "temperature", "max_tokens", "reasoning_effort", "history"}
    )
    ...
    def __setattr__(self, name, value):
        if name in self._BROADCAST_ATTRS:
            for s in self._sessions:
                v = (
                    openai_tools_to_claude(value)
                    if name == "tools" and type(s) is NativeClaudeSession
                    else value
                )
                setattr(s, name, v)
        else:
    ...
    def _raw_ask(self, *args, **kwargs):
        ...
        test_error = lambda x: (
            isinstance(x, str) and x.lstrip().startswith(("!!!Error:", "[Error:"))
        )
        for attempt in range(self._retries + 1):
            ...
            if not is_err:
                if attempt > 0:
                    ...
                    isinstance(last_chunk, str)
                    and "[!!! 流异常中断" in last_chunk
                    and n > 1
                ):
                    self._cur_idx = (idx + 1) % n
                    self._switched_at = time.time()
                    print(
                        f"[MixinSession] Partial failure, next call → s{self._cur_idx} ({self._sessions[self._cur_idx].name})"
                    )
                return return_val
...
class NativeToolClient:
    ...
    def _thinking_prompt():
        return (
            THINKING_PROMPT_EN
            if os.environ.get("GA_LANG") == "en"
            else THINKING_PROMPT_ZH
        )

    def __init__(self, backend):
        self.backend = backend # 真实模型后端
        # 给后端设置默认 system prompt，要求模型每轮输出 summary，并且未完成任务时要调用工具
        self.backend.system = self._thinking_prompt()
        self.name = self.backend.name  # 模型名字
        self._pending_tool_ids = [] # 记录上一轮模型要求调用的工具 id

    def set_system(self, extra_system):
        """合并系统提示词:
        
        项目系统提示词\n\n行动规范：每次回复要 summary，未完成任务必须工具调用
        """
        combined = (
            f"{extra_system}\n\n{self._thinking_prompt()}"
            if extra_system
            else self._thinking_prompt()
        )
    ...
    def chat(self, messages, tools=None):
        ...
        # 把 TOOLS_SCHEMA 挂到底层 backend
        if tools:
            self.backend.tools = tools
        # 如果是新会话，清空 pending tool id
        ...
        for tr in tool_results:
            ...
            else: # 如果没有 tool_use_id，就退化为普通文本
                combined_content = [
                    {"type": "text", "text": f"<tool_result>{content}</tool_result>"}
                ] + combined_content
        
        # 补齐遗漏的 pending tool result
        ...
        # 保存下一轮 pending tool ids
        # 下一轮把工具执行结果传回模型时，要用这些 id 对应
